...
def freeze_all_except_fc(model):
    ...
    Args:
        model: PyTorch model (ECGSMARTNET)
    ...
    print("\n" + "="*80)
    print("TRANSFER LEARNING: Layer Freezing Status")
    print("="*80)
    ...
    for name, param in model.named_parameters():
        if param.requires_grad:
            ...
            print(f"✓ TRAINABLE: {name:40s} - {param.numel():,} parameters")
        else:
...
<<<<<<< Updated upstream
    ...
    if not PRETRAINED_ACS_MODEL_PATH:
        print("\n" + "!"*80)
        print("ERROR: Please set PRETRAINED_ACS_MODEL_PATH in the script!")
        print("Example: PRETRAINED_ACS_MODEL_PATH = 'models/ecgsmartnet_ACS_2024-01-01.pt'")
        print("!"*80 + "\n")
        exit(1)
    
    if not os.path.exists(PRETRAINED_ACS_MODEL_PATH):
        print(f"\n{'!'*80}")
        print(f"ERROR: Pretrained model not found at: {PRETRAINED_ACS_MODEL_PATH}")
        print(f"{'!'*80}\n")
        exit(1)
...
>>>>>>> Stashed changes
    ...
    print(f'\n{"="*80}')
    print(f'TRANSFER LEARNING: ACS → CAD')
    print(f'{"="*80}')
...
<<<<<<< Updated upstream
            ...
            model = torch.load(PRETRAINED_ACS_MODEL_PATH, map_location=device)
            
            # Freeze all layers except the final FC layer
            model = freeze_all_except_fc(model)
            model = model.to(device)
...
<<<<<<< Updated upstream
            ...
            # Only optimize the trainable parameters (final FC layer)
...
<<<<<<< Updated upstream
    ...
    plt.suptitle(f'Transfer Learning Best Model Performance (Iteration {global_best_hyperparams["iteration"]}, Optimal Threshold={optimal_threshold:.3f})\n' + 
    ...
    # Save plot
    plot_path = f'models/ecgsmartnet_CAD_transfer_BEST_MODEL_iter{global_best_hyperparams["iteration"]:03d}_{global_best_hyperparams["time"]}_curves.png'
