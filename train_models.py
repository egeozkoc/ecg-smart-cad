...
if __name__ == '__main__':
    ...
    # Choose between 'random' and 'grid' search
    SEARCH_MODE = 'random'  # Options: 'random' or 'grid'
    # ===========================================================================
    ...
    print(f'\n{"="*80}')
    print(f'HYPERPARAMETER SEARCH MODE: {SEARCH_MODE.upper()}')
    print(f'{"="*80}')
    ...
    if SEARCH_MODE == 'random':
        # Random search: continuous distributions
        ...
        raise ValueError(f"Invalid SEARCH_MODE: {SEARCH_MODE}. Must be 'random' or 'grid'.")
    ...
    # ============= SETUP HYPERPARAMETER ITERATION =============
    if SEARCH_MODE == 'random':
        ...
        # Create iterator for random search
        search_iterations = range(1, n_random_search + 1)
        total_iterations = n_random_search
        
    elif SEARCH_MODE == 'grid':
        total_iterations = len(lr0s) * len(lrs) * len(bss) * len(wds)
        ...
        # Create grid search iterator with enumeration
        grid_combinations = list(itertools.product(lr0s, lrs, bss, wds))
    
    # ============= MAIN HYPERPARAMETER SEARCH LOOP =============
    count_search = 0
    
    if SEARCH_MODE == 'random':
        iterator = search_iterations
    else:  # grid search
        iterator = range(len(grid_combinations))
    ...
    for idx in iterator:
        ...
        # ============= SAMPLE/SELECT HYPERPARAMETERS =============
        if SEARCH_MODE == 'random':
            # Set seed for reproducible hyperparameter sampling
            np.random.seed(idx * 42)  # Unique seed per iteration
            ...
            lr0, lr, bs, wd = grid_combinations[idx]
        
        # ============= PRINT ITERATION INFO =============
        print(f'\n{"="*80}')
        print(f'{SEARCH_MODE.upper()} SEARCH ITERATION: {count_search}/{total_iterations}')
        ...
        try:
            ...
            # Set WandB project name based on search mode
            wandb_project = f'ecgsmartnet-cad-{SEARCH_MODE}-search'
            ...
            for epoch in range(num_epochs):
                ...
                if val_loss < best_val_loss:
                    ...
                    model_filename = 'models/ecgsmartnet_CAD_{}_iter{:03d}_{}.pt'.format(SEARCH_MODE, count_search, current_time)
    ...
    # Save plot
    plot_path = f'models/ecgsmartnet_CAD_{SEARCH_MODE}_BEST_MODEL_iter{global_best_hyperparams["iteration"]:03d}_{global_best_hyperparams["time"]}_curves.png'
